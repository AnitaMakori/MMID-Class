...
#MODEL PARAMETERS
lat_par<-base_par
lat_par['lat_cov_i'] <- 0.8 # intervention coverage of latrines (*3 times from baseline) 
lat_par['lat_eff'] <- 0.7 # effectiveness of latrines
lat_par
...
############################
vac_par<-base_par
vac_par['vac_cov_i'] <- 0.2 # Vaccination coverage
vac_par['vac_eff'] <- 0.3 # Vaccination effectiveness
vac_par
...
############################
#Costs
C_latrine <- 200          # Cost of latrine per unit ($US)
Num_household <- initP/5  # Number of household in the population (average 5 people per household) 
C_vaccine<- 350           # Cost of vaccine per person ($US)
C_medical <- 30           # Average medical cost of hepatitis E per case ($US)
...
#Health outcomes
YLD.case<- 0.037 # Life year with disability per case (disability weight at 0.3 for 45 days, {0.3*45/365})
YLL.death<- 30    # Life years lost per death
